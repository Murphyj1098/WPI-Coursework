...
void rtinit2() {
    ...
    printf("At time t=%f, node 2 initial distance vector: %d %d %d %d\n",
        clocktime, dt2.costs[0][0], dt2.costs[1][1], dt2.costs[2][2], dt2.costs[3][3]);    
    ...
    for(int i=0; i < MAX_NODES; i++)
    {
        ...
        if(pkt.mincost[i] > neighbor2->NodeCosts[i])
            pkt.mincost[i] = neighbor2->NodeCosts[i];
    }

    // send packets
    for(int i=0; i < MAX_NODES; i++)
    {
        if(neighbor2->NodeCosts[i] != INFINITY && i != NODE)
        {
            pkt.destid = i;
            toLayer2(pkt);
...
void rtupdate2( struct RoutePacket *rcvdpkt ) {
    ...
    for(int i=0; i < MAX_NODES; i++)
    {
        ...
        for(int j=0; j < MAX_NODES; j++)
        {
            if(dt2.costs[i][j] < minVal[i])
                minVal[i] = dt2.costs[i][j];
        }
    }
    ...
    if(dt2.costs[source][source] > rcvdpkt->mincost[NODE])
    {
        dt2.costs[source][source] = rcvdpkt->mincost[NODE];
        updateTable = 1;
    }
